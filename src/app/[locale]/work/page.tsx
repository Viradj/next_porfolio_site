...
export default function Work() {
    ...
    const fetchData = async (date) => {
        ...
        try {
            ...
            const adjustedPrices = prices.map(entry => {
            ...
            const avgPrice = adjustedPrices.reduce((sum, entry) => sum + entry.price, 0) / adjustedPrices.length;
            setAveragePrice(avgPrice);

            const labels = adjustedPrices.map(entry => entry.hour);
            const values = adjustedPrices.map(entry => entry.price);
            const colors = adjustedPrices.map(entry =>
            ...
            const table = adjustedPrices.map(entry => ({
                ...entry,
    ...
    return (
        <Flex fillWidth maxWidth="m" direction="column" alignItems="center" padding="l">
            ...
            {/* Date Selector */}
            <div style={{ marginBottom: '1rem', display: 'flex', gap: '1rem', flexDirection: 'column', alignItems: 'center' }}>
                <label style={{ color: '#90a4ae', fontSize: '1rem' }}>
                    Select Date:
                </label>
                <DatePicker
            ...
            {loading ? (
                ...
                <>
                    ...
                    {/* Table Display */}
                    <table style={{ marginTop: '2rem', width: '100%', maxWidth: '600px', color: '#eceff1', borderCollapse: 'collapse' }}>
                        <thead>
                        ...
                        <tbody>
                            {tableData.map((entry, index) => (
                                <tr key={index} style={{ borderBottom: '1px solid #37474f' }}>
                                    ...
                                    <td style={{ padding: '0.5rem', color: entry.difference > 0 ? 'red' : 'green' }}>
